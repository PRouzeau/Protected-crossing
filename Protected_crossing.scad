...
inf_text = true;
//fr:Affiche un bus en train de tourner pour vérification passages
//Display a turning bus to check clearances
Display_bus = true;
//fr:Bus position en X
//Bus X position
X_bus = 5.2; //[-12:0.1:12]
//fr:Bus position en Y
//Bus Y position
Y_bus = -4; //[-12:0.1:12]
//fr:Angle d'orientation du bus
//Bus orientation angle (clockwise)
Ang_bus = 27;
//fr:Affiche une voiture en train de tourner pour vérification passages
//Display a turning car to check clearances
Display_car = false;
//fr:Voiture position en X
//Car X position
X_car = 2.5; //[-12:0.1:12]
//fr:Voiture position en Y
//Car Y position
Y_car = 5.5; //[-12:0.1:15]
//fr:Angle d'orientation de la voiture
...
basetext = [
	"Specification:",
//fr::"Spécification",	
	str("X road branch A footprint: ",XA_totwidth,"m" ),
//fr::	str("Emprise route X branche A: ",XA_totwidth,"m" ),
	str("X road branch B footprint: ",XB_totwidth?XB_totwidth:XA_totwidth,"m" ),
//fr::	str("Emprise route X branche B: ",XB_totwidth,"m" ),
	str("Y road branch A footprint: ",YA_totwidth,"m" ),
//fr::	str("Emprise route Y branche A: ",YA_totwidth,"m" ),
	str("Y road branch B footprint: ",YB_totwidth?YB_totwidth:YA_totwidth,"m" ),
//fr::	str("Emprise route Y branche B: ",YB_totwidth,"m" ),
	str("X road branch A lane width: ",round(Wlane_wd2(vX,vA)/10)/100,"m" ),
//fr::str("Route X branche A largeur voie: ",round(Wlane_wd2(vX,vA)/10)/100,"m" ),	
  str("X road branch B lane width: ",round(Wlane_wd2(vX,vB)/10)/100,"m" ),
//fr::str("Route X branche B largeur voie: ",round(Wlane_wd2(vX,vB)/10)/100,"m" ),	
  str("Y road branch A lane width: ",round(Wlane_wd2(vY,vA)/10)/100,"m" ),
//fr::str("Route Y branche A largeur voie: ",round(Wlane_wd2(vY,vA)/10)/100,"m" ),	
  str("Y road branch B lane width: ",round(Wlane_wd2(vY,vB)/10)/100,"m" )
//fr::str("Route Y branche B largeur voie: ",round(Wlane_wd2(vY,vB)/10)/100,"m")
];

function roundabout_text() = round_int_diam?["Roundabout:",str("- Internal diameter: ",_round_int_diam," m"),str("- External diameter: ",round(round_road_radius()/cfu*10)/10,"m"),str("- Bikepath internal diameter: ",round((round_road_radius()-roundabout_space)*2/cfu*10)/10,"m")]:[]; 	
//fr::function roundabout_text() = round_int_diam?["Rond-point:",str("- Diamètre interne: ",_round_int_diam," m"),str("- Diamètre externe: ",round(round_road_radius()/cfu*10)/10,"m"),str("- Diamètre interne piste cyclable: ",round((round_road_radius()-roundabout_space)*2/cfu*10)/10,"m")]:[]; 
