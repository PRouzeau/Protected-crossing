...
Road protected crossing design, 'the dutch way'
 For state of the art explanation, have a look here:
	https://bicycledutch.wordpress.com/2011/04/07/state-of-the-art-bikeway-General-or-is-it/
	
See Readme_first.txt file for details:	
...
//fr:Merci de ne pas modifier les variables de cet onglet
//Please do not modify variables of this tab
//_=true;
//Version
pcross_version = 1;
//language
language = "en";
//fr::language = "fr";
//fr:Coefficient d'unités principal, natif en mm, interface en m
//Main unit coefficient: native is m m:interface is m
cfu=1000;
//fr:Coefficient d'unités pour les lignes
//Line unit coefficient: native is mm 
cfu_line=1;
...
/*[Display]*/
/*
//fr:Affiche batiments (pas encore installé)
//Display buildings - not implemented yet
display_building = false;
*/
//fr:Projection (pour exportation fichier dxf)
//Projection (for dxf file export)
Projection=false;
//fr:Affiche la route (sinon seulement trottoirs et marquage)
//Display the road (else only marking)
Display_road = true;
//fr:Affiche avertissements et données dans la vues 3D
//Display information and warning in the 3D view
Disp_text = true;
//fr:Affiche avertissements dans la console
//Display information and warning in the console
inf_text = true;
//fr:Afficher les véhicules
//Display vehicles
vh_disp = false;	
//== VEHICLES ===================
...
/*[Vehicles]*/
//-- Vehicle 1 -------------------
//fr:Type du véhicule 1 à afficher
//Type of vehicle 1 to display
vh1_type = 1; // [0:"None",1:"Red car",2:"Blue car",3:"Bus", 4:"Bicyclist"]
//fr::vh1_type = 1; // [0:"Aucun",1:"Voiture rouge",2:"Voiture bleue",3:"Bus",4:"Cycliste"]

//fr:Éléments accessoires du véhicule 1
//Vehicle 1 accessory elements
...
//fr:Véhicule 1 position en X
//Vehicle 1, X position
vh1_X = 2.8; //[-22:0.1:22]
//fr:Véhicule 1 position en Y
//Vehicle 1, Y position
vh1_Y = 6; //[-22:0.1:22]
//fr:Véhicule 1 angle
//Vehicle 1 angle
vh1_ang = 115;
//-- Vehicle 2 -------------------
//fr:Type du véhicule 2 à afficher
//Type of vehicle 2 to display
vh2_type = 2; // [0:"None",1:"Red car",2:"Blue car",3:"Bus", 4:"Bicyclist"]
//fr::vh2_type = 2; // [0:"Aucun",1:"Voiture rouge",2:"Voiture bleue",3:"Bus",4:"Cycliste"]

//fr:Éléments accessoires du véhicule 2
//Vehicle 2 accessory elements
...
//fr:Véhicule 2 position en X
//Vehicle 2, X position
vh2_X = 8; //[-22:0.1:22]
//fr:Véhicule 2 position en Y
//Vehicle 2, Y position
vh2_Y = 2.5; //[-22:0.1:22]
//fr:Véhicule 2 angle
//Vehicle 2 angle
vh2_ang = 180;
//-- Vehicle 3 -------------------
//fr:Type du véhicule 3 à afficher
//Type of vehicle 3 to display
vh3_type = 3; // [0:"None",1:"Red car",2:"Blue car",3:"Bus", 4:"Bicyclist"]
//fr::vh3_type = 3; // [0:"Aucun",1:"Voiture rouge",2:"Voiture bleue",3:"Bus",4:"Cycliste"]

//fr:Éléments accessoires du véhicule 3
//Vehicle 3 accessory elements 
...
//fr:Véhicule 3 position en X
//Vehicle 3, X position
vh3_X = -3.4; //[-22:0.1:22]
//fr:Véhicule 3 position en Y
//Vehicle 3, Y position
vh3_Y = -5.2; //[-22:0.1:22]
//fr:Véhicule 3 angle
//Vehicle 3 angle
vh3_ang = -52;
//-- Vehicle 4 -------------------
//fr:Type du véhicule 4 à afficher
//Type of vehicle 4 to display
vh4_type = 4; // [0:"None",1:"Red car",2:"Blue car",3:"Bus", 4:"Bicyclist"]
//fr::vh4_type = 4; // [0:"Aucun",1:"Voiture rouge",2:"Voiture bleue",3:"Bus",4:"Cycliste"]

//fr:Éléments accessoires du véhicule 4
//Vehicle 4 accessory elements
...
//fr:Véhicule 4 position en X
//Vehicle 4, X position
vh4_X = 4.4; //[-22:0.1:22]
//fr:Véhicule 4 position en Y
//Vehicle 4, Y position
vh4_Y = 8.8; //[-22:0.1:22]
//fr:Véhicule 4 angle
//Vehicle 4 angle
vh4_ang = 155;
...
/*fr:[Général]*/
/*[General]*/
//fr:Coté de conduite à droite
//Road driving side on right
right_drive = true;
//-------------
//fr:Il y a des feux de circulation
// Traffic light presence
traffic_light=true;
//------------
//fr:Quelle route a priorité
...
t_cross = false;
//fr:Le feu est installé après le passage piéton (quand on le regarde)
//Traffic light is installed after pedestrian crossing (looking at it) 
light_after_crossing = true; 
//fr:Décalage du centre de rayon des coins de trottoir (augmente le rayon mais diminue l'espace pour les piétons)
//Pavement corner center offset (increase radius, but decrease pedestrian passage)
_corner_offset = 0.2; //[0:0.1:3]
corner_offset = _corner_offset*cfu; 
//fr:Angle de raccordement de la deviation dans le carrefour, reduire donne une route plus directe 
//fr:Les îlots de parking sont en béton (sinon ils sont peints)
//Parking islands are in concrete (else they are painted)
parking_island_concrete = false;
//fr:Longueur de la branche de déviation voiture (depuis le début de la route, pas du centre du carrefour)
//car deviation length (from road start, not from crossing center)
_dev_length = 16;
dev_length = _dev_length*cfu;
//fr:Longueur de la branche de déviation vélo (depuis le début de la route, pas du centre du carrefour)
//cycle way deviation length (from road start, not from crossing center)
_cydev_length = 10;
...
/*[Main lanes count]*/
//fr:Route X branche A: nombre de voies à droite
//X road A branch, lanes number on right side
XAr_nb_lanes = 2; // [0:1:4]
//fr:Route X branche A, nombre de voies à gauche
//X road A branch, lanes number on left side
XAl_nb_lanes = 1; // [0:4]
//fr:Route X branche B, voies à droite
//X road B branch, right lanes
XBr_nb_lanes = 2; // [0:4]
//fr:Route X branche B, voies à gauche
//X road B branch, left lanes
XBl_nb_lanes = 1; // [0:4]
//fr:Route Y branche A: voies à droite
//Y road A branch, right lanes
YAr_nb_lanes = 1; // [0:4]
//fr:Route Y branche A, voies à gauche
//Y road A branch, left lanes
YAl_nb_lanes = 1; // [0:4]
//fr:Route Y branche B, voies à droite
//Y road B branch, right lanes
YBr_nb_lanes = 1; // [0:4]
//fr:Route Y branche B, voies à gauche
//Y road B branch, left lanes
YBl_nb_lanes = 1; // [0:4]
...
//Misc dimensions
//fr:hauteur du trottoir - depuis la route
//pavement height - from road level
_pavht=0.15; 
pavht=_pavht*cfu; 
//number of segment for pavement cylinder ends of 300mm diam. This is key to overall calculation time ??
pavseg=12;
//===============================
/*fr:[Route X, parties communes]*/
/*[Road X common elements]*/
//Right and left are from road when looking the crossing
//yet crossing is assumed to be straight and mirrorable
//fr:Largeur de la route X (y compris les trottoirs)
//X road width (includes pavement and all)
XA_totwidth= 20; //[4:0.2:50]
//fr:La branche B de la route X est en continuité de la branche A (même largeur, même trottoir, etc.)
//X road branch B is in continuity with branch A (same width, pavement, etc.)
XX = true; // [true]
//fr:Trottoir de séparation médian
//Central separation pavement
XA_central = 0; //[0:0.1:2]
//fr:Largeur passage piéton (0: pas de passage)
//Pedestrian crossing width (0: no crossing)
XA_pedcross_wd = 2.5; // [0,2.5,3]
//fr:Distance du passage piéton au bord de la route perpendiculaire
//Pedestrian crossing distance (from other road side)
XA_pedcross_shift = 3; // [0.4:0.2:6]
//fr:Angle de raccordement pour la déviation des pistes, le réduire diminue le décalage.
//Connecting angle for crossing swerve, reducing gives a more direct route.
Xway_angle=45; //[15:5:45]
//fr:Types de flèches au sol branche A (remplir tableau valeurs:"vers droite","tout droit","tout droit et droite")
//Road marking arrows branch A (fill in array, values "right","straight","straight right")
XAr_lane_arrows = ["right","straight","straight","straight"];
//fr::XAr_lane_arrows = ["vers droite","tout droit","tout droit","tout droit"];
...
XBr_lane_arrows = ["right","straight","straight","straight"]; 
//fr::XBr_lane_arrows = ["vers droite","tout droit","tout droit","tout droit"];
//["straight","right","straight right"]
//fr:permet une déviation automatique, branche A (pour optimiser l'espace intérieur)
//allow automatic deviation, branch A (to optimize internal space)
XA_allow_dev=true;
//fr:permet une déviation automatique, branche B (pour optimiser l'espace intérieur)
//allow automatic deviation, branch B (to optimize internal space)
XB_allow_dev=true;

//==============================
/*fr:[Route X coté droite, branche A]*/
/*[Road X right side, branch A]*/
//fr:Largeur trottoir - au carrefour
//Pavement width - at crossing
XAr_pavement = 2; // [0.5:0.1:6]
//fr:Largeur allée de coté (pour voitures ou vélos)
//Alley width: alley could be for cars or cycling
XAr_alley = 2; // [0:0.1:6]
//Is the alley a cycle path (-> change color)
//fr:Allée entièrement réservée à la piste cyclable
//Alley is dedicated to cycling
XAr_cycle_path=true; 
//fr:La piste cyclable est en double sens
//Cycle path in double direction
...
//fr:Distance immeuble/trottoir (pas installé)
//Distance between building and road (Not implemented)
XAr_building = 0;
*/

//===============================
/*fr:[Route X coté gauche, branche A]*/
/*[Road X left side, branch A]*/
//fr:Largeur trottoir - au carrefour
//Pavement width - at crossing
XAl_pavement = 2;  // [0.5:0.1:6]
//fr:Largeur contre-allée
//Alley width: alley could be for cars or cycling
XAl_alley = 2; // [0:0.1:6]
//fr:La contre-allée est une piste cyclable
//Is alley a cycle path (-> change color)
XAl_cycle_path=true; 
//fr:La piste cyclable est à double sens
//Cycle path in double direction
...
//Bus lane
XAl_bus_lane = false;
//fr:Voie de bus sur la branche B
//bus lane on B branch
XBl_bus_lane = false;
...
*/
//===================================
/*fr:[Route Y, parties communes]*/
/*[Road Y common elements]*/
//fr:Largeur de la route Y (y compris les trottoirs)
//Road Y total width (includes pavement)
YA_totwidth= 16; //[4:0.2:50]
//fr:La branche B de la route Y est en continuité de la branche A (même largeur, même trottoir, etc.)
//Y road branch B is in continuity with branch A (same width, pavement, parking lane, etc.)
YY = true; // [true]
//fr:Trottoir de séparation médian
//Central separation pavement
...
//fr:Distance du passage piéton au bord de la route perpendiculaire
//Pedestrian crossing position (from other road side)
...
///fr:Types de flèches au sol branche en face (B) (remplir tableau valeurs:"vers droite","tout droit","tout droit et droite")
//Road marking arrows facing branch (B)-(fill in array, values "right","straight","straight right")
YBr_lane_arrows = ["","straight","straight","straight"];
//fr::YBr_lane_arrows = ["vers droite","tout droit","tout droit","tout droit"];
//fr:permet une déviation automatique, branche A (pour optimiser l'espace intérieur)
//allow automatic deviation, branch A (to optimize internal space)
YA_allow_dev=true;
//fr:permet une déviation automatique, branche B (pour optimiser l'espace intérieur)
...
//===================================
/*fr:[Route Y coté droite, branche A]*/
/*[Road Y right side, branch A]*/
//fr:Largeur trottoir - au carrefour
//Pavement width - at crossing
...
*/
//===================================
/*fr:[Route Y coté gauche, branche A]*/
/*[Road Y left side, branch A]*/
//fr:Largeur trottoir - au carrefour
//Pavement width - at crossing
...
//fr:Largeur ligne centrale
//Road central line thickness
_road_cent_line = 150; // 3 u
//fr:Largeur ligne séparation de voies (mm)
//Road separation lane line thickness (mm)
_road_sep_line = 100; // 2 u
//fr:Largeur ligne voie bus (mm)
//Bus lane line thickness (mm)
_bus_lane_line = 250; // 5 u
//fr:Largeur ligne bande cyclables  (mm)
//Cycle lane line thickness  (mm)
...
//fr:Largeur ligne séparation double sens piste cyclable (mm)
//cycle double way separation line (mm)
_cycle_cent_line = 90; //3u (u=30 for cycle)
//-- Stop line intervals if dashed ---
//fr:Intervalle entre marques de ligne de stop piéton (mm)
//Pedestrian stop line interval (mmm)
_stop_line_lg = 1000;
//fr:Espace entre marques de ligne de stop piéton (mm)
//Stop line space while dashed (0 if not) (mm)
_stop_line_sp =200; //[0,200]
// according Vienna convention
//fr:Largeur ligne de stop passage piéton  (mm)
//Stop line thickness: 500 or 600  (mm)
_stop_line_thk =500; //[500,600]
//fr:Le passage piéton est de type 'Zèbre'
//Pedestrian crossing is zebra type
pedcross_zebra = true;
//fr:Largeur lignes passages piéton en zèbre (mm)
//Pedestrian zebra crossing line width (mm)
_ped_zebra_line = 500;
//fr:Largeur lignes latérale passages sans 'zèbre' (mm)
//Side line width for non zebra pedestrian crossings (mm)
...
//fr:Rayon virage camions (trottoir pleine hauteur)
//Truck radius turn (full height pavement)
_truck_radius = 9; //[8:0.2:12]
truck_radius = _truck_radius*cfu;
//fr:Augmentation de la largeur des pistes à l'intérieur du carrefour
//Width extension of cycle way when inside crossing
_cycle_wd_extent_crossing = 0.25; //[0:0.05:0.80]
cycle_wd_extent_crossing = _cycle_wd_extent_crossing*cfu;
//fr:Distance du trottoir au rayon de virage théorique (vers l'intérieur)
//Clearance between real pavement side and theoretical turn (to the inside)
_radius_clearance = 0.25; //[0.1:0.1:0.8]
radius_clearance = _radius_clearance*cfu;
//---------------------------------
//fr:Distance de la ligne de stop au passage piéton (m)
//Pedestrian crossing stop line distance (m)
_stop_line = 4;
stop_line = _stop_line*cfu;
//fr:Distance du poteau de feu au bord du trottoir (m)
// Traffic light pole distance from pavement side (m)
...
//fr:Correction position 1 ilot au coin C1
//Island position 1 correction at corner C1
C1_island_pos1 = 0; //[-1.5:0.1:2.5]
//fr:Correction position 2 ilot au coin C1
//Island position 2 correction at corner C1
C1_island_pos2 = 0; //[0:0.1:2.5]
//-----------
//fr:Correction position 1 ilot au coin C2
//Island position 1 correction at corner C2
C2_island_pos1 = 0; //[-1.5:0.1:2.5]
//fr:Correction position 2 ilot au coin C2
//Island position 2 correction at corner C2
C2_island_pos2 = 0; //[0:0.1:2.5]
//------------------------------
//fr:Correction position 1 ilot au coin C3
//Island position 1 correction at corner C3
C3_island_pos1 = 0; //[-1.5:0.1:2.5]
//fr:Correction position 2 ilot au coin C3
//Island position 2 correction at corner C3
C3_island_pos2 = 0; //[0:0.1:2.5]
//---------------
//fr:Correction position 1 ilot au coin C4
//Island position 1 correction at corner C4
C4_island_pos1 = 0; //[-1.5:0.1:2.5]
//fr:Correction position 2 ilot au coin C4
//Island position 2 correction at corner C4
C4_island_pos2 = 0; //[0:0.1:2.5]

//fr:Ajustement position feu piste double, coin C1
//double cycle path light position adjust, corner C1
C1_dbl_light_adj = 0; //[0:0.1:2.5]
//fr:Ajustement position feu piste double, coin C2
//double cycle path light position adjust, corner C2
C2_dbl_light_adj = 0; //[0:0.1:2.5]
//fr:Ajustement position feu piste double, coin C3
//double cycle path light position adjust, corner C3
C3_dbl_light_adj = 0; //[0:0.1:2.5]
//fr:Ajustement position feu piste double, coin C4
//double cycle path light position adjust, corner C4
...
//fr:Couleur de la route
//Road color
color_road = "dimgray";
//Cycle lane or path color
//color_cycle = "firebrick";
//fr:Couleur des voies cyclables
//Cycle way color
color_cycle = [0.78,0.1,0.1];
//fr:Couleur bord de trottoir
//Pavement border color
...
//fr:Couleur espace route/bâtiments
//Land color in front of building
...
module cutcorner (a, b, bottom=-100, ht=500, extent=true) {
	...
	lgseg1prev = (borderprev-poslane1prev-dyextprev-dyint1prev)/sin(wayangp);
	lgseg2prev = (borderprev-poslane2prev-dyextprev-dyint2prev)/sin(wayangp);

	lgstrsegprev = (pos1xuprev+pos2xuprev)-(rad_extprev*2+radius1uprev+radius2uprev)*sin(wayangp)-(lgseg1prev+lgseg2prev)*cos(wayangp);

	strshiftprev = pos1xuprev-(rad_extprev+radius1uprev)*sin(wayangp)-lgseg1prev*cos(wayangp)-lgstrsegprev/2;
//top corner
	t(strshift) 
		t(lgstrseg/2,border,bottom) {
				mirrorx() mirrory() 
					seg(way_tot, rad_ext, ht, false, wayang);
			if(extent)
				cubez(4000,way_tot,ht,-2000,-way_tot/2);
		}
	//right corner
	t(0,-strshiftprev)
		t(borderprev,lgstrsegprev/2,bottom) {
			rotz(-90)mirrory() 
				//mirrorx() mirrory() 
					seg(way_totprev, rad_extprev, ht, false, wayangp);
			if(extent)
				cubez(way_totprev,4000,ht,-way_totprev/2,-2000);
		}
	// straight junction
	hull(){
		t(strshift)
			t(lgstrseg/2,border,bottom)
				t(0,-rad_ext)
					rotz(-wayang)
						cubez(1,way_tot,ht, 0,rad_ext-way_tot/2);
		t(0,-strshiftprev) 
			t(borderprev, lgstrsegprev/2,bottom)
				t(-rad_extprev,0)
					rotz(wayangp) 
						cubez(way_totprev,1,ht,rad_extprev-way_totprev/2);
	}//hull()
}//cutcorner()
...
basetext = [
	"Specification:",
//fr::"Spécification",	
traffic_light?"There is traffic light":str("NO traffic light",road_priority==1?", X road have priority":road_priority==2?", Y road have priority":", no road have priority"),
//fr::  traffic_light?"Il y a des feux de trafic":str("PAS de feux de trafic",road_priority==1?", la route X a priorité":road_priority==2?", la route Y a priorité":", aucune route prioritaire"),
	str("X road footprint branch A/B: ",XA_totwidth,"/",XB_totwidth?XB_totwidth:XA_totwidth," m" ),
//fr::	str("Emprise route X branche A/B: ",XA_totwidth,"/",XB_totwidth?XB_totwidth:XA_totwidth,"m" ),
	str("Y road footprint branch A/B: ",YA_totwidth,"/",YB_totwidth?YB_totwidth:YA_totwidth," m" ),
//fr::	str("Emprise route Y branche A/B: ",YA_totwidth,"/",YB_totwidth?YB_totwidth:YA_totwidth," m" ),

  str("X road branch A/B lane width: ",round100(Wlane_wd2(vX,vA)),"/",round100(Wlane_wd2(vX,vB))," m" ),
//fr::str("Route X branche A/BB largeur voie: ",round100(Wlane_wd2(vX,vA)),"/",round100(Wlane_wd2(vX,vB)),"m" ),	
  str("Y road branch A/B lane width: ",round100(Wlane_wd2(vY,vA)),"/",round100(Wlane_wd2(vY,vB))," m"),
//fr::str("Route Y branche A/BB largeur voie: ",round100(Wlane_wd2(vY,vA)),"/",round100(Wlane_wd2(vY,vB))," m" ),

  str("X road bikeway branchA right/left: ",round100(Wcycle_wd(vX,vA,vright)),"/",round100(Wcycle_wd(vX,vA,vleft))," m"),
//fr::str("Route X voie cyclable br.A droite/gauche: ",round100(Wcycle_wd(vX,vA,vright)),"/",round100(Wcycle_wd(vX,vA,vleft))," m"),
	str("Y road bikeway branchA right/left: ",round100(Wcycle_wd(vY,vA,vright)),"/",round100(Wcycle_wd(vY,vA,vleft))," m"),
//fr::  str("Route Y br.A droite/gauche: ",round100(Wcycle_wd(vY,vA,vright)),"/",round100(Wcycle_wd(vY,vA,vleft))," m"),
	str("Corner passage: ",corner_txt)
//fr::  str("Passage au coin: ",corner_txt)
];

function roundabout_text() = round_int_diam?["Roundabout:",str("- Internal diameter: ",_round_int_diam," m"),str("- External diameter: ",round100(d_roundline)," m"),str("- Bikepath internal distance: ",round100(d_roundline+roundabout_space*2)," m")]:[]; 	
...
spectxt = concat(
	basetext,
	roundabout_text(),
	str("Bikepath radius: ",radius_txt)
//fr::str("Rayon piste cyclable: ",radius_txt)  	
);
...
// Select according t_cross	
		...
		rotroad(axis,branch) {
			if (side==vright) {
			 ...
			 //-- Main pavement corners --
				if(!(t_cross&&branch==vB))
					pav_corner2(axis, branch);
				else {// pavement junction on T
					//enlarge to deviation and pavement sep ???
					pvwd = Wpavement[vX][vA][vleft];
					cywd = Wcycle_wd(vX,vA,vleft);
					t(-totwidth[vY][vB]/2-1000,totwidth[vX][vA]/2) {
						pavement(pvwd, totwidth[vY][vB]+2000);
						t(2600,-pvwd-cywd)
							pavement(200, totwidth[vY][vB]-3200);
						// cycle path junction on T
						if(disp_road)
							color(color_cycle)
								cubex(totwidth[vY][vB]+2000,cywd,22, 0,-pvwd-cywd/2,8);
					}
				}
			}	
			//All side pavement
	...
	module all_pav(a,b,side) {
		devpav=Wpavdev[a][b][side];
		tot2 = totwidth[a][b]/2;
		perp2 = perpwidth[a][b]/2;
		cycledouble = Wcycle_double[a][b][side];
		cyclepath=Wcycle_path[a][b][side];
		pav = Wpavement[a][b][side];
		pavalley = Wpav_alley[a][b][side];
		Ualley = Walley[a][b][side];
		park = Wpark_lane[a][b][side];

		pedshift = Wpedcross_shift[a][b];
		ped = Wpedcross_wd[a][b];
		wayang = way_ang(a,b);
		upvst_r = crn_xr (a,b);
		upvst_l = crn_xl (a,b);
		upvst = side?upvst_l:upvst_r;
		...
		mirrory(side) {
			color(color_pavement)
				...
				if(Ualley)
					t(perp2+pedshift+ped, tot2-pav-Ualley){
						...
						border_start = cr_border_r(a,b)-perpwidth[a][b]/2+250;
			...
			if(disp_road && cyclepath) {
				startcycle=t_cross&&a==vX&&((b==vA&&side==vleft)||(b==vB&&side==vright))?0:perp2;
				color(color_cycle) 
					side_alley(10,22);
		...
		central = Wcentral[a][b];
		coord = Wmain_start[a][b][vright]-Wnb_lanes[a][b][vright]*Wlane_wd2(a,b)-central/2;
		if (Wnb_lanes[a][b][vleft]>=2 && Wnb_lanes[a][b][vright] && side==vright) {
			wdc2 = (Wlane_wd2(a,b)-2700);
			...
			color (color_pavement) {
				...
				// before ped crossing
				hull(){
					t(perp2+pedshift-150,coord+devped2)
						dmirrory() duplx(-200)
							cylz(300,pavht,0,wdc2++central/2-150,0,pavseg);
				}
			}
...
module seg (width, radius, depth=10, intrad=true, angle=45) {
...
// position is the segment AXIS	start if internal, but EXTERNAL for external radius
	...
	t(0,radius+(intrad?1:0)*width/2)
		diff(){
			cylz(2*radius+addext,depth, 0,0,0, 48);
			//::::
			cylz(2*radius+addint,40+depth, 0,0,-20, 48);  
			cubez(80000,80000,40+depth, 40000,0,-20);
			rotz(-angle)
				cubez(80000,80000,40+depth, -40000,0,-20);
		}
}
...
- The internal radius is based upon main radius but could be increased to have a more round crossing, at the cost of motorised vehicles space reduction. This will give a more 'roundabout' look.
	...
	What is drafted is the right path of the given branch (crossing path). corner 1 is the entering corner, corner2 is the exiting corner.
*/
	rht = 22; // road elevation (to mask other stuff)
	lht = 8; // line elevation 
	wayang = way_ang(a,b);
	seg_shift = cycle_wd_extent_crossing/sin(wayang);
	ap = paxis(a);
	bp = pbranch(a,b);
	bopp= oppbranch(b);
	...
	enlarg = cycle_wd_extent_crossing/2;
	...
	radius1u = rturn(radius1[a][b],wayang);
	radius2u = rturn(radius1[ap][bp],wayang);
	radius = avg_radius(a,b);
	pos1xu= crn_xr(a,b);
	pos2xu= crn_xl(a,bopp);
	devp = isdev(ap,bp);
	rad_ext = cr_rad_ext(a,b);
	rad_mean = rad_ext-way_tot/2;

	border1 = cr_border1(a,b);
	border2 = cr_border2(a,b);
	//echo(border1 =border1,border2=border2);
	//this is perpendicular branch border
	...
	if(priority && disp_road)
			...
			if(cycle_priority)
				...
				dmirrorx() 
					t(-lgstrseg/2,border,-4)
						mirrory() 
							seg(way_tot, rad_ext, rht, false, wayang);
				 // middle straights  
				t(0,border-way_tot/2)
					cubez(lgstrseg,way_tot,rht);
			}
			//right seg
			t(pos1xu,poslane1-way_wd/2,-4) 
				duplx(-seg_shift)
					seg(way_wd, radius1u,rht, true, wayang);
			//left seg 
			t(-pos2xu,poslane2-way_wd/2,-4) 
				duplx(seg_shift)
					mirrorx() 
						seg(way_wd, radius2u,rht, true, wayang);
		...
		//echo(lgseg1 =lgseg1 ,lgseg2 =lgseg2 );
		//bias right
		t(strshift+lgstrseg/2+dxext,border-dyext,2) {
			rotz(-wayang){
				if(priority&&disp_road)
					color(color_cycle)
						cubex(lgseg1,way_tot,rht, 0,-way_tot/2,rht/2-2); 
				white() {
					cubex(lgseg1,cycle_cross_line,lht, 0,cycle_cross_line/2,lht/2+2);
					cubex(lgseg1,cycle_cross_line,lht, 0,-way_tot-cycle_cross_line/2,2);
				}
			}
		}
		//bias left
		t(strshift-lgstrseg/2-dxext,border-dyext,2) {
			rotz(wayang){
				if(priority&&disp_road)
					color(color_cycle)
						cubex(-lgseg2,way_tot,rht, 0,-way_tot/2,rht/2-2); 
				white() {
					cubex(-lgseg2,cycle_cross_line,lht, 0,cycle_cross_line/2,lht/2+2);
					cubex(-lgseg2,cycle_cross_line,lht, 0,-way_tot-cycle_cross_line/2,2);
				}
			}
		}
	//-- cycle lane markings --------  
		...
		t(strshift)
			dmirrorx()
				t(-lgstrseg/2,border-way_tot/2,5)
					mirrory() 
						diff() {
							seg(way_tot+cycle_cross_line*2, rad_mean-way_tot/2-cycle_cross_line, lht, true, wayang);
							t(0,0,-10)
								seg(way_tot, rad_mean-way_tot/2,30,true,wayang);
						}
		//---------------------
		//right seg 
		t(pos1xu,total/2-Wpavement[a][b][vright]-way_wd/2,5)
			diff(){
				duplx(-seg_shift) 
					seg(way_wd+cycle_cross_line*2, radius1u-cycle_cross_line,lht, true, wayang);
				duplx(-seg_shift)
					seg(way_wd, radius1u,30, true, wayang);
			}
		//left seg 
		t(-pos2xu,total/2-Wpavement[a][b?0:1][vleft]-way_wd/2,5) 
			mirrorx()
				diff(){
					duplx(-seg_shift)
						seg(way_wd+cycle_cross_line*2, radius2u-cycle_cross_line,lht, true, wayang);
					duplx(-seg_shift)
						seg(way_wd, radius2u,30, true, wayang);
				}
		if(double)
			t(usewdp/2,border-way_tot/2,rht)
				mirrorx() mirrory()
					seg(cycle_cent_line, radius+way_wd/2-enlarg, lht, true, wayang);
	...
	//---------------------------------
		...
		t(strshift,border-way_tot/2)  {
			dmirrory()
				t(-lgstrseg/2-5,way_wd/2+enlarg+5)
					line2(cycle_cross_line-6,0,lgstrseg-10,1,1000,500,lht);	
		// mark separator line ???
			if(double)
				t(-usewdp/2)
					line2(cycle_cent_line,0,usewdp,0,1000,0,rht+lht);
	  ...
	  pedcross_shift= Wpedcross_shift[axis][branch];
		...
		white() {
			if (width) { //if pedestrian cross
				t(pos+5,start+10)
					rotz(-90) {
						if (pedcross_zebra) //is zebra type - reduced in width to avoid merging in projection 
							line2(width-10,0,length-10,1,2*ped_zebra_line,ped_zebra_line,34); 
						else 
							duply (width-pedcross_side_line-10)
								line2(pedcross_side_line,0,length-10,1,1000,ped_zebra_line,34); 
					}
			//dashed stop line ???
...
module disp_lanes () {
	...
	for(axis=[0:1],branch=[0:1])
		...
		rotroad(axis,branch) {
			...
			roadx = perpwidth[axis][branch]/2;
			pcross_shift = Wpedcross_shift[axis][branch];
			pcross_wd = Wpedcross_wd[axis][branch];		
			Wlane_sep(roadx,pcross_shift,pcross_wd, Wmain_start[axis][branch], Wnb_lanes[axis][branch], Wcentral[axis][branch], Wlane_wd2(axis,branch), dev);
			wayang = way_ang(axis, branch);
			axisp = paxis(axis);
			bprev = prevbranch(axis, branch);
			// park and cycle lanes
			for(side=[0:1]) {
				lane_str = crn_xr(axis, branch);
				lane_stl = crn_xl(axis, branch);
				//move toward crossing of 700 mm is an arbitrary choice to improve tangency of deviated arrival lanes. it seems to work ok for all angle range (15-45°).
				lane_st = (side?lane_stl:lane_str)-700;
				...
				mirrory(side) {
					//-- Parking lanes ---------
					t(roadx, rstart)
						dpark_lane(roadx,pk_lane,axis,branch,side, deviate, dev);
					//-- Cycle lanes -----------
					if(cy_lane) //there's cycle lane
						t(lane_st, rstart-pk_lane)
							dcycle_lane(lane_st,cy_lane,pcross_shift,pcross_wd,side, pk_lane,dlength, dev2, dev);
				}
			}
		}
	}
				...
				// place separations
					...
					if(!parking_island_concrete) {	
						...
						intersection() {
							...
							duplx(800,26)
								t(-width/1.6) rotz(45)
									cubez(100,width*1.5,10, 0,-width/1.6,-6);
						}
					}
				}//white()
				if(parking_island_concrete) 
	...
	module dcycle_lane (road, width, pcross_shift, pcross_wd=2500, side = vright, park_lane=0, dlength, dev2, dev) {
		...
		if(disp_road)
			color(color_cycle)
				line2(width-cycle_lane_line,-500,dlength,-1,10,0,15,dev2);
		white(){//lines
			duply(-width) {
...
//green() cylz (200,2000,0,bordery);
 ...
 quart_bias=round_int_diam?round_bias*0.707:0; 	
  ...
  if (!(t_cross&&branch==vB)&&Wcycle_wd(axis,branch,vright)) {
		...
		istruckpav = (xext+yext)>15000&&!quart_bias;
...
module quart_shape (radius,wdx,wdy,way,ht=pavht, extent=-1400, diag=0) {
		...
		t(wdx-radius-diag, wdy-radius) 
			...
			if(diag)	{
				...
				hull() {
					t(wdx-radius-diag, wdy-radius)
						rotz(45)
							cubez(way,10, ht, radius-way/2,cuty,0);
					t(wdx-radius, wdy-radius-diag)
						rotz(-45)
							cubez(10,way, ht, cutx,radius-way/2,0);
				}
			}
			if(!cuty)	
...
module disp_text () {
	...
	module dpt (x,y,txt) {
		t(x,y)
			mirrorx(!right_drive) 
				scale([1,2,1]) text(text=txt, size=800, halign = "center", valign = "center", $fn=pr);
	}
// We shall extrude text for projection, as 2D text kills the projection process (?)
	if (Projection||!$preview)
		render() linear_extrude(10) txt();
	else
		txt();
	module txt () {
		// User text
...
module check () {
	...
	nbranch = ["A","B"];
	nside = [" right"," left"];
//fr::nside = [" droite"," gauche"];	
	function typeway(axis,branch, side) =	str("Road ",naxis[axis],",branch ",nbranch[branch],nside[side]);
//fr::function typeway(axis,branch, side) =	str("Voie ",naxis[axis],",branche ",nbranch[branch],nside[side]);
	//-----------------------
...
//-- Check cycle lane width ------
 // ruled width is INSIDE marking
	...
	function c_pavalley (a,b,s) = 
		Wcycle_path[a][b][s]&&Walley[a][b][s]&&Wpark_lane[a][b][s]?real_alley_pav[a][b][s]<600?str(er_pavalleypark, real_alley_pav[a][b][s]/cfu," m, ", typeway(a,b,s),"\n."):"":"";

	function c_1lane_sep (a,b) =
		Wcentral[a][b]&&Wnb_lanes[a][b][vleft]<2&&!round_int_diam?str(er_1lane_sep, typeway(a,b,vleft),"\n.\n"):"";

}//check()
//== Warnings =====================
wr_path_width = "* Minimum recommended width for a\n cycle path is 2m, preferred 2.5m.\n Present width:";
//fr::wr_path_width = "* La largeur minimale recommandée\n pour une piste cyclable unidirectionnelle\n est de 2m, avec une préférence pour 2.5m. Largeur actuelle:";
//---------------
wr_2path_width = "* Minimum recommended width for a\n double direction cycle path is 2.5m,\n preferred 3m. Present width:";
//fr::wr_2path_width = "* La largeur minimale recommandée pour une piste cyclable a double sens est de 2.5m, avec une préférence pour 3m. Largeur actuelle:";
//-----------------
wr_lane_width = "* Minimum recommended width for a\n cycle lane is 1.5m within marking line (which is 250mm wide) s, preferred 2m. Present width:";
//fr::wr_lane_width_fr = "* La largeur minimale recommandée pour une bande cyclable est de 1.5m à l'INTERIEUR du marquage (250mm), avec une préférence pour 2m. Largeur actuelle:";
//--------------------
wr_lane_park_width = "* Minimum recommended width for a\n cycle lane along a parking lane is 1.75m\n INSIDE marking (250mm), with a preference for 2m.\n Present width:";
//fr::wr_lane_park_width = "* La largeur minimale recommandée pour une bande cyclable le long d'une file de stationnement est de 1.75m à l'INTERIEUR du marquage (250mm), avec une préférence pour 2m. Largeur actuelle:";
//---------------------------------
wr_light_ped_crossing = "* When there is traffic lights, you generally\n find pedestrian crossings.";
//fr::wr_light_ped_crossing = "* Lorqu'il y a des feux rouges, il y a généralement des passages piétons.";
//---------------------------------
//== ERRORS =======================
// a mettre en oeuvre
er_1lane_sep = "Error!: You can't set a central pavement\n separation when there is only one lane\n, this prevent trucks and busses to turn:\n";
//fr::er_1lane_sep = "Erreur!: vous ne pouvez pas mettre de\n séparateur central béton quand il\n n'y  a qu'une seule voie, ceci empêche\n les bus et les camions de tourner:\n";

er_pavalleypark = "Error!: when a parking lane is located\n aside an alley pavement separation,\n this separation shall have minimum width\n for proper car loading/unloading and\n protection of alley users, actual pavement\n width is:";
//fr::er_pavalleypark = "Erreur!: quand une voie de parking est\n située a coté d'un trottoir de séparation\n d'une allée, ce trottoir doit être\n suffisamment large pour un chargement/\ndéchargement correct des voitures et\n pour la protection des usagers de l'allée.\n Largeur actuelle du trottoir:\n";
...
module multiLine (lines, size=1000, wdtxt=25000, always=false){
  ...
  t(!right_drive?-wdtxt:0)
		if(Disp_text||always)
			union(){
				for(i=[0:len(lines)-1])
					translate([0 , -i *size*1.5*(i?1:1.2), 0 ])  text(lines[i], size*(i?1:1.2), $fn=pr);
			}
}
